class HomeController < ApplicationController
  require 'csv'
  def home
    @text_file = TextAssets.new
  end

  def text_extract
    #    puts "@"*100
    file_path = "#{Rails.root}/tmp/emails/emails#{Time.now.strftime("%Y%m%d%H%M%S")}.csv"
    text = params[:text]
    #    puts "@"*100
    data_list = []
    data_list << ["email"]
...
#    File.delete(file_path) if File.exist?(file_path)
  ...
  def file_extract
    @text_file = TextAssets.new(:document=>params[:text][:document])
    @text_file.valid?
